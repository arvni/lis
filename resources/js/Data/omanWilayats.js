// Governorates of Oman and their wilayats.
// Source: official administrative divisions of Oman (11 governorates, 63 wilayats)
// https://en.wikipedia.org/wiki/Provinces_of_Oman
export const omanWilayats = {
    Muscat: ['Muscat', 'Muttrah', 'Bawshar', 'A’Seeb', 'Al Amerat', 'Qurayyat'],
    Dhofar: [
        'Salalah',
        'Taqah',
        'Mirbat',
        'Rakhyut',
        'Dhalkut',
        'Thumrait',
        'Maqshin',
        'Shaleem and the Hallaniyat Islands',
        'Sadah',
        'Al Mazyunah',
    ],
    Musandam: ['Khasab', 'Bukha', 'Dibba', 'Madha'],
    'Al Buraimi': ['Al Buraimi', 'Mahdah', 'As-Sunaynah'],
    'Al Dakhiliyah': [
        'Nizwa',
        'Bahla',
        'Manah',
        'Adam',
        'Izki',
        'Samail',
        'Al Hamra',
        'Bidbid',
        'Jebel Akhdar',
    ],
    'Al Dhahirah': ['Ibri', 'Dhank', 'Yanqul'],
    'Al Sharqiyah North': [
        'Ibra',
        'Al Mudhaibi',
        'Bidiya',
        'Al Qabil',
        'Wadi Bani Khalid',
        'Dema Wa Thaieen',
        'Sinaw',
    ],
    'Al Sharqiyah South': [
        'Sur',
        'Jalan Bani Bu Ali',
        'Jalan Bani Bu Hassan',
        'Masirah',
        'Al Kamil Wal Wafi',
    ],
    'Al Batinah North': ['Sohar', 'Shinas', 'Liwa', 'Saham', 'Al Khaboura', 'Suwaiq'],
    'Al Batinah South': ['Rustaq', 'Barka', 'Nakhal', 'Al Awabi', 'Wadi Al Maawil', 'Al Musanaah'],
    'Al Wusta': ['Haima', 'Duqm', 'Al Jazer', 'Mahout'],
};
