...
const FileChip = ({ document, onClick }) =>
    document ? (
        <Chip
            icon={<AttachFileIcon />}
                                    ...
                                    >
                                        ...
                                        >
                                            ...
                                            <Chip
                                                ...
                                                size="small"
                                                color={
                                                    APPROVAL_COLORS[approval.status] ?? 'default'
                                                }
                ...
                actions={
                    <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap' }}>
                        <Button
                            ...
                            onClick={() =>
                                router.visit(
                                    route('inventory.purchase-requests.create', {
                                        repeat_from: pr.id,
                                    }),
                                )
                            }
                        ...
                        {currentStatus === 'DRAFT' && (
                            <Button
                                ...
                                variant="outlined"
                                onClick={() =>
                                    router.visit(route('inventory.purchase-requests.edit', pr.id))
                                }
                            >
                                Edit
                            </Button>
                        ...
                        {currentStatus === 'SUBMITTED' &&
                            ...
                            !(approvals ?? []).some(
                                ...
                                <Button
                                    ...
                                    onClick={() =>
                                        router.post(
                                            route('inventory.purchase-requests.recall', pr.id),
                                        )
                                    }
                                >
                                    Recall
                        ...
                        {['ORDERED', 'PAID', 'SHIPPED', 'PARTIALLY_RECEIVED'].includes(
                            currentStatus,
                        ) && (
                        ...
                        {canReceive(currentStatus) && (
                            <Button
                                ...
                                onClick={() =>
                                    router.visit(
                                        route('inventory.purchase-requests.receive', pr.id),
                                    )
                                }
                            >
                                Receive Items
                            ...
                            <Grid container spacing={2}>
                                <Grid size={{ xs: 12, sm: 6 }}>
                                    ...
                                    <InfoRow label="Workflow">
                                        {pr.workflow_template?.name ?? '— none —'}
                                    </InfoRow>
                                    ...
                                    <InfoRow
                                        ...
                                        chipContent={
                                            poDocument ? (
                                                <FileChip
                                                    document={poDocument}
                                                    onClick={() => setViewingDoc(poDocument)}
                                                />
                                            ) : null
                                        }
                                    />
                                        ...
                                        chipContent={
                                            paymentDocument ? (
                                                <FileChip
                                                    ...
                                                    onClick={() => setViewingDoc(paymentDocument)}
                                                />
                                            ) : null
                                        }
                                    />
                                    ...
                                    <TableBody>
                                        {pr.receipts.map((receipt) => (
                                            <TableRow key={receipt.id}>
                                                <TableCell>
                                                    {receipt.created_at?.substring(0, 10)}
                                                </TableCell>
                                                <TableCell>
                                                    <Button
                                                        size="small"
                    ...
                    <Card>
                        ...
                        <CardContent sx={{ p: 0 }}>
                            {histories.length === 0 ? (
                                ...
                                <Timeline
                                    sx={{ '& .MuiTimelineItem-root:before': { flex: 0, p: 0 } }}
                                >
                                    {histories.map((h, idx) => {
                    ...
                    {matchLoading ? (
                        ...
                        <>
                            <Box sx={{ mb: 2, p: 1.5, bgcolor: 'grey.50', borderRadius: 1 }}>
                                ...
                                <Typography variant="body2">
                                    <b>Urgency:</b> {matchResult.pr.urgency}
                                </Typography>
                                <Typography variant="body2">
                                    <b>Est. Total:</b> {matchResult.pr.estimated_total}
                                </Typography>
                                <Typography variant="body2">
                                    <b>Requester roles:</b>{' '}
                                    {matchResult.pr.requester_roles.join(', ') || '—'}
                                </Typography>
                            </Box>
                            ...
                            <Box
                                sx={{
                                    ...
                                    borderRadius: 1,
                                    border: '1px solid',
                                    borderColor: matchResult.matched_template
                                        ? 'success.main'
                                        : 'warning.main',
                                    bgcolor: matchResult.matched_template
                                        ? 'success.50'
                                        : 'warning.50',
                                }}
                            >
                                {matchResult.matched_template ? (
                                    <>
                                        <Typography
                                            ...
                                            fontWeight={700}
                                        >
                                            ✓ Matched: {matchResult.matched_template.name}
                                        </Typography>
                                        {pr.workflow_template_id !==
                                            matchResult.matched_template.id && (
                                            <Typography
                                                variant="caption"
                                                color="warning.main"
                                                display="block"
                                                sx={{ mt: 0.5 }}
                                            >
                                                ⚠ Current assigned template differs — re-submit to
                                                update.
                                            </Typography>
                                        )}
                                    </>
