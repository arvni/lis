...
const CellInput = memo(({ value, onChange, type = 'text', highlight }) => (
    <input
        ...
        onBlur={(e) =>
            (e.target.style.border = highlight ? '1px solid #d32f2f' : '1px solid transparent')
        }
    />
));
...
const ItemImport = () => {
    ...
    const handleSubmit = () => {
        ...
        setSubmitting(true);
        router.post(
            route('inventory.items.import.rows'),
            { rows: validRows },
            {
                onError: (errs) => {
                    setErrors(errs);
    ...
    return (
        <>
                        ...
                        {rows.length > 0 && !parsing && (
                            <>
                                ...
                                {invalidCount > 0 && (
                                    <Chip
                                        icon={<ErrorIcon />}
                                        label={`${invalidCount} missing name`}
                                        color="error"
                                        size="small"
                                        variant="outlined"
                                    />
            ...
            {/* ── Action bar ── */}
            <Box
                sx={{ display: 'flex', gap: 1.5, alignItems: 'center', mb: 1.5, flexWrap: 'wrap' }}
            >
            ...
            <TableContainer component={Paper} sx={{ maxHeight: '62vh' }}>
                ...
                >
                    <TableHead>
                        <TableRow>
                            <TableCell
                                sx={{ px: 0.5, py: 0.75, bgcolor: 'background.paper', width: 36 }}
                            />
                            <TableCell
                                sx={{
                                    px: 0.5,
