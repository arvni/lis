...
const BarcodeInput = ({
    ...
    const scan = async (barcode) => {
        ...
        setStatus('scanning');
        try {
            const { data } = await axios.get(route('api.inventory.barcode.scan'), {
                params: { barcode },
            });
            if (data.found) {
                setStatus('found');
    ...
    const endIcon = () => {
        ...
        if (status === 'found')
            return (
                <Tooltip title="Barcode matched — fields auto-filled">
                    <CheckCircleIcon fontSize="small" color="success" />
                </Tooltip>
            );
        if (status === 'new')
            return (
                <Tooltip title="New barcode — fill details manually">
