    ...
    if (loading)
        return (
            <Box sx={{ py: 1, display: 'flex', alignItems: 'center', gap: 1 }}>
                ...
                <Typography variant="caption">Loading FIFO…</Typography>
            </Box>
        );
            ...
            >
                {data.shortfall > 0 && (
                    <Alert
                        severity="error"
                        icon={<WarningAmberIcon fontSize="small" />}
                        sx={{ py: 0.5, fontSize: '0.75rem', borderRadius: 0 }}
                    >
                        Shortfall: {data.shortfall.toFixed(4)} base units (have{' '}
                        {data.available.toFixed(4)}, need {data.needed.toFixed(4)})
                    </Alert>
                )}
                <Table size="small">
                    <TableHead>
                        <TableRow
                            sx={{
                                '& th': { fontSize: '0.7rem', py: 0.5, bgcolor: 'action.hover' },
                            }}
                        >
                            ...
                            return (
                                <TableRow key={i} sx={{ '& td': { fontSize: '0.72rem', py: 0.4 } }}>
                                    ...
                                    <TableCell align="right">
                                        {lot.quantity_base_units.toFixed(4)}
                                    </TableCell>
                                    <TableCell
                                        align="right"
                                    ...
                                    <TableCell
                                        ...
                                        sx={{
                                            color:
                                                lot.remaining_after <= 0
                                                    ? 'text.disabled'
                                                    : 'inherit',
                                        }}
                                    >
                                        {lot.remaining_after.toFixed(4)}
                                    </TableCell>
                                </TableRow>
                            );
                        })}
