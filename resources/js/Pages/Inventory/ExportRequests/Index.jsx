...
import { EXPORT_REQUEST_STATUSES, STATUS_COLORS } from './constants';

const ExportFilter = ({ defaultFilter, onFilter }) => {
...
const ExportRequestsIndex = () => {
    ...
    const handlePageReload = useCallback(
        (page, filters, sort, pageSize) => {
            router.visit(route('inventory.export-requests.index'), {
    ...
    const handleBulkApprove = () => {
        router.post(
            route('inventory.export-requests.bulk-approve'),
            { ids: selected },
            ...
            { field: 'created_at', headerName: 'Created', width: 110 },
            {
                ...
                getActions: ({ row }) => [
                    <GridActionsCellItem
                        ...
                        showInMenu
                        onClick={() =>
                            router.visit(route('inventory.export-requests.show', row.id))
                        }
                    />,
                ],
            },
            ...
            <PageHeader
                ...
                actions={
                    <Button
                        ...
                        variant="contained"
                        color="success"
                        onClick={() => router.visit(route('inventory.export-requests.create'))}
                    >
                        New Request
                    </Button>
                }
...
const breadcrumbs = [
    ...
    { title: 'Export Requests', link: null },
];

ExportRequestsIndex.layout = (page) => (
    <AuthenticatedLayout auth={page.props.auth} breadcrumbs={breadcrumbs}>
        {page}
    </AuthenticatedLayout>
);

export default ExportRequestsIndex;
