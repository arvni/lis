...
const MethodFields = ({
    ...
    const validate = () => {
        ...
        if (type === 'PANEL') {
            ...
            if (type === 'TEST') {
                if (!m.turnaround_time || m.turnaround_time < 1)
                    errs.turnaround_time = 'Turnaround time is required';
        ...
        <Box>
            ...
            <Box>
                {methodTests.map((item, index) => {
                    ...
                    const price =
                        (m.price_type ?? 'Fix') === 'Fix'
                            ? m.price
                                ? `$${m.price}`
                                : '—'
                            : 'Variable';
                    const refPrice =
                        (m.referrer_price_type ?? 'Fix') === 'Fix'
                            ? m.referrer_price
                                ? `$${m.referrer_price}`
                                : '—'
                            : 'Variable';
