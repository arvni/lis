...
export default function PricingConfig({
    ...
    const addParam    = useCallback(() => patch({ parameters: [...parameters, { id: makeId(6), value: "" }] }), [patch, parameters]);
    const updateParam = useCallback((id, val) => patch({ parameters: parameters.map((p) => (p.id === id ? { ...p, value: val } : p)) }), [patch, parameters]);
    const removeParam = useCallback((id) => patch({ parameters: parameters.filter((p) => p.id !== id) }), [patch, parameters]);

    const addCond    = useCallback(() => patch({ conditions: [...conditions, { id: makeId(6), condition: "", value: "" }] }), [patch, conditions]);
    const updateCond = useCallback((id, field, val) => patch({ conditions: conditions.map((c) => (c.id === id ? { ...c, [field]: val } : c)) }), [patch, conditions]);
    const removeCond = useCallback((id) => patch({ conditions: conditions.filter((c) => c.id !== id) }), [patch, conditions]);

    const setFormula  = useCallback((val) => patch({ formula: val }), [patch]);
    const insertParam = useCallback((name) => setFormula(formula + (formula.trim() ? " + " : "") + name), [setFormula, formula]);
            ...
            if (priceType === "Formulate") {
                ...
                for (let i = 0; i < conditions.length; i++) {
                    ...
                    try {
                        ...
                        const met = new Function(...keys, `"use strict"; return !!(${c.condition})`)(...keys.map((k) => scope[k]));
                        if (met) {
                            setTestResult(`Condition ${i + 1} matched — ${parseFloat(mathjs.evaluate(c.value || "0", scope)).toFixed(3)} OMR`);
                            return;
                        }
    ...
    const renderTester = () => {
        ...
        return (
            <Box sx={{ pt: 1, borderTop: "1px solid", borderColor: "divider" }}>
                <Typography variant="subtitle2" color="text.secondary" sx={{ mb: 1 }}>Test</Typography>
                <Box sx={{ display: "flex", flexWrap: "wrap", gap: 1.5, alignItems: "flex-end" }}>
                    {paramNames.map((name) => (
                        <TextField
                            key={name} size="small" label={name} type="number"
                    ...
                    <Button variant="contained" size="small" startIcon={<PlayArrow />} onClick={runTest}>
                        Run
                    </Button>
                </Box>
                {testResult && <Alert severity="success" sx={{ mt: 1.5, py: 0.5 }}>{testResult}</Alert>}
                {testError  && <Alert severity="error"   sx={{ mt: 1.5, py: 0.5 }}>{testError}</Alert>}
            </Box>
        );
    };
    ...
    return (
        <Stack gap={3}>
            ...
            <ToggleButtonGroup
                ...
                sx={{mb: 1.5, "& .MuiToggleButtonGroup-grouped": { border: "1px solid", borderColor: "divider" }}}
            >
            ...
            {priceType === "Fix" && (
                <FormControl fullWidth>
                    <InputLabel error={Boolean(error)} required>Amount</InputLabel>
                    <OutlinedInput
                        type="number" label="Amount" value={price ?? 0} error={Boolean(error)}
                        slotProps={{ htmlInput: { min: 0, step: 0.001 } }}
                        onChange={(e) => onPriceChange(e.target.value)}
                        endAdornment={
                            <InputAdornment position="end">
                                <Typography variant="caption" color="text.secondary">OMR</Typography>
                            </InputAdornment>
                        }
                    />
                    ...
                    <Box>
                        <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", mb: 1 }}>
                            <Typography variant="subtitle2" color="text.secondary">
                                Conditions <Typography component="span" variant="caption">(first match wins)</Typography>
                            </Typography>
                            <Button size="small" startIcon={<Add />} onClick={addCond}>Add</Button>
                        </Box>
                            ...
                            <Stack gap={1}>
                                {conditions.map((c, idx) => (
                                    <Box key={c.id} sx={{ display: "flex", gap: 1, alignItems: "center" }}>
                                        ...
                                        <TextField
                                            size="small" fullWidth label="When true"
                                            value={c.condition}
                                            placeholder="e.g. age > 18"
                                            onChange={(e) => updateCond(c.id, "condition", e.target.value)}
                                            sx={{ "& .MuiInputBase-root": { fontFamily: "monospace" } }}
                                        />
                                        <TextField
                                            size="small" fullWidth label="Price"
                                            value={c.value}
                                            placeholder="e.g. 100 or age * 2"
                                            onChange={(e) => updateCond(c.id, "value", e.target.value)}
                                            sx={{ "& .MuiInputBase-root": { fontFamily: "monospace" } }}
                                        />
                                        <IconButton size="small" color="error" onClick={() => removeCond(c.id)}>
                                            <Delete fontSize="small" />
                                        </IconButton>
                                    </Box>
                                ))}
