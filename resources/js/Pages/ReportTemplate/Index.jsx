...
const Index = () => {
    ...
    const findReportTemplate = useCallback(
        (id) => reportTemplates.data.find((reportTemplate) => reportTemplate.id === id),
        [reportTemplates],
    );

    const editReportTemplate = useCallback(
        (id) => () => {
            let _report_template = findReportTemplate(id);
            setReportTemplate({
                id: _report_template.id,
                name: _report_template.name,
                approval_flow_id: _report_template.approval_flow_id,
    ...
    const printReportTemplate = useCallback(
        (id) => () => window.open(route('reportTemplates.show', id), '_blank'),
        [],
    );

    const deleteReportTemplate = useCallback(
        (id) => () => {
    ...
    const handleDestroy = useCallback(
        () =>
            router.post(
                route('reportTemplates.destroy', reportTemplate.id),
                { _method: 'delete' },
    ...
    const columns = useMemo(
        () => [
            ...
            },
            {
                ...
                width: 180,
                sortable: false,
                valueGetter: (_, row) => row.approval_flow?.name ?? 'Single approval',
            },
            {
                field: 'id',
                headerName: 'Action',
    ...
    return (
        <>
            ...
            {openDeleteForm && (
                <DeleteForm
                    title={`${reportTemplate?.name} ReportTemplate`}
