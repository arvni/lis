...
const PendingApprovalsWidget = ({ count }) => {
    ...
    return (
        <Card
            sx={{ mb: 3, borderLeft: '4px solid', borderColor: 'warning.main', cursor: 'pointer' }}
            onClick={() =>
                router.visit(
                    route('inventory.purchase-requests.index', { filters: { view: 'approval' } }),
                )
            }
        >
            <CardContent
...
const Index = ({ data, date, pendingPRApprovals }) => {
    const handleRefresh = (date = '') =>
        router.visit(route('dashboard'), { data: { date }, only: ['data', 'date'] });
    return (
        <>
