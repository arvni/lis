...
const AddSection = ({ open, onClose, sectionWorkflow, onChange, setSectionWorkflow }) => {
    ...
    const addParameter = () => {
        if (checkParameter()) {
            ...
            let tmp = { ...parameter };
            delete tmp.index;
            if (parameter.index != null) parameters[parameter.index] = tmp;
            else parameters.push(tmp);
            setSectionErrors({});
    ...
    const changeParameter = (e) =>
        setParameter((prevState) => ({ ...prevState, [e.target.name]: e.target.value }));
    const changeRequiredParameter = (e) =>
    ...
    const checkParameter = () => {
        ...
        if (
            sectionWorkflow.parameters.filter(
                (item, index) => item.name === parameter.name && index !== parameter.index,
            ).length
        )
            Object.assign(tmp, { name: 'Please Enter Different Parameter Title' });
        if (parameter.name === '') Object.assign(tmp, { name: 'Please Enter Parameter Title' });

        if (
            parameter.type === 'options' &&
            (parameter.options === '' ||
                Array.from(new Set(parameter.options?.split(';').map((item) => item.trim())))
                    .length < 2)
        )
    ...
    const checkForm = () => {
        let tmp = {};
        if (!sectionWorkflow.section.id) Object.assign(tmp, { section: 'Please Select A Section' });
        ...
        <Dialog open={open} onClose={onClose} maxWidth="md" fullWidth>
            ...
            <DialogContent>
                <Grid container sx={{ py: '1em' }} spacing={2}>
                    ...
                    {sectionWorkflow?.section?.sectionGroup && (
                        <Grid size={{ xs: 12, sm: 6 }}>
                            <SelectSearch
                ...
                <List dense sx={{ width: '100%' }}>
                    {sectionWorkflow.parameters.map((parameter, index) => (
                        <React.Fragment key={index}>
                            ...
                            >
                                <ListItemText
                                    primary={`${parameter.name} : ${parameter.type}`}
                        ...
                        <Grid container spacing={1} sx={{ width: '100%' }}>
                            ...
                            <Grid size={{ xs: 12, sm: 6, md: 4, lg: 3 }}>
                                <FormControl variant="outlined" fullWidth>
                                    <InputLabel id="parameter-type-label" variant="outlined">
                                        Parameter Type
                                    </InputLabel>
                                    <Select
                                ...
                                <FormControlLabel
                                    control={
                                        <Switch
                                            ...
                                            onChange={changeRequiredParameter}
                                            name={'required'}
                                        />
                                    }
                                    label="Required"
