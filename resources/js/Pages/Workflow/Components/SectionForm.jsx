...
import {
    alpha, Box, Button, Chip, FormControlLabel,
    FormHelperText, Grid, IconButton, Paper,
    Switch, TextField, Tooltip, Typography, useTheme,
} from "@mui/material";
...
const TYPE_COLOR = {
    text: "default", number: "primary", date: "info",
    time: "secondary", options: "warning", file: "success",
};
...
const ParameterRow = ({param, index, isEditing, onEdit, onDelete}) => {
    ...
    return (
        <Box sx={{
            ...
            {param.required && (
                <Chip label="req" size="small" variant="outlined"
...
const SectionForm = ({sectionWorkflow, setSectionWorkflow, onSubmit, onClose}) => {
    ...
    const sectionGroupChange = (e) =>
        setSectionWorkflow(prev => ({...prev, section: {sectionGroup: e.target.value, name: "", id: ""}}));

    const sectionChange = (e) =>
        setSectionWorkflow(prev => ({...prev, section: {...prev.section, ...e.target.value}}));

    /* ── Parameter handlers ── */
    const changeParam = (e) => setParameter(prev => ({...prev, [e.target.name]: e.target.value}));
    ...
    const checkParameter = () => {
        ...
        else if (sectionWorkflow.parameters.some((p, i) => p.name === parameter.name && i !== parameter.index))
            err.name = "Title must be unique";
        if (parameter.type === "options") {
            const opts = parameter.options?.split(";").map(s => s.trim()).filter(Boolean) ?? [];
    ...
    const editParameter = (i) => {
        setParameter(parameter.index === i ? EMPTY_PARAM : {...sectionWorkflow.parameters[i], index: i});
    };
        ...
        >
            ...
            <Box sx={{p: 2.5}}>
                    ...
                    {(sectionWorkflow?.section?.sectionGroup || sectionWorkflow?.section?.id) && (
                        <Grid size={{xs: 12, sm: 6}}>
                            <SelectSearch
                                ...
                                fullWidth
                                label="Section"
                                defaultData={sectionWorkflow?.section?.sectionGroup?.id
                ...
                {sectionErrors.section && (
                    <FormHelperText error sx={{mb: 1.5}}>{sectionErrors.section}</FormHelperText>
                )}

                {/* ── Divider ── */}
                <Box sx={{
                    my: 2.5, borderTop: `1px dashed ${theme.palette.divider}`,
                }}/>
                ...
                {sectionWorkflow.parameters.length > 0 ? (
                    ...
                    <Box sx={{
                        mb: 2, py: 2, textAlign: "center",
                        border: `1px dashed ${theme.palette.divider}`,
                        borderRadius: 1,
                        bgcolor: alpha(theme.palette.action.hover, 0.3),
                    }}>
                        <TuneIcon sx={{fontSize: 28, color: alpha(theme.palette.text.secondary, 0.3), mb: 0.5}}/>
                ...
                {sectionErrors.parameters && (
                    <FormHelperText error sx={{mb: 1.5, mt: -1}}>{sectionErrors.parameters}</FormHelperText>
                )}
                ...
                <Box sx={{
                    ...
                    <Typography variant="caption" fontWeight={600} color="text.secondary"
                                display="block" sx={{mb: 1.5, textTransform: "uppercase", letterSpacing: "0.08em"}}>
                        {isEditingParam ? `Editing: ${sectionWorkflow.parameters[parameter.index]?.name}` : "New Parameter"}
                    </Typography>

                    <Grid container spacing={1.5} sx={{mb: 1.5}}>
                        <Grid size={{xs: 12, sm: "auto"}} sx={{flex: "1 1 180px"}}>
                            <TextField
                                label="Parameter name" fullWidth size="small"
                                name="name"
                        ...
                        {parameter.type === "options" && (
                            <Grid size={{xs: 12, sm: "auto"}} sx={{flex: "1 1 200px"}}>
                                <TextField
                                    label="Options" fullWidth size="small"
                                    name="options"
                    ...
                    {/* Type chips */}
                    <Box sx={{mb: 1.5}}>
                        <Typography variant="caption" color={parameterErrors.type ? "error" : "text.disabled"}
                    ...
                    <Box sx={{display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 1}}>
                        <FormControlLabel
                            control={
                                <Switch size="small" checked={parameter.required} onChange={toggleRequired}/>
                            }
                            label={
                                <Typography variant="caption" fontWeight={500}>Required</Typography>
                            }
                        />
                        <Box sx={{display: "flex", gap: 1}}>
                            {isEditingParam && (
                                <Button size="small" onClick={cancelEditParam} startIcon={<CloseIcon/>}>
                                    Cancel edit
                                </Button>
                            )}
                            <Button
