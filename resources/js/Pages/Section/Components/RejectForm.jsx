...
const DoneForm = ({ open, onClose, acceptanceItemState, onChange, onSubmit, options = [] }) => {
    ...
    const check = () => {
        ...
        if (!acceptanceItemState.details) tmp = { ...tmp, details: `Please Enter Details` };
    ...
    return (
        <Dialog open={open} onClose={onClose}>
            <DialogTitle>
                Please fill form below to submit the section {acceptanceItemState?.section?.name}{' '}
                has been done
                <Divider />
            </DialogTitle>
            <DialogContent>
                <Grid container spacing={2}>
                    ...
                    <Grid>Test Name : {acceptanceItemState.test?.name}</Grid>
                </Grid>
