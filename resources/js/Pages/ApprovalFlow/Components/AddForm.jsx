        ...
        <Stack spacing={3} sx={{width: "100%"}}>
            ...
            <Paper elevation={0} sx={{p: 3, border: '1px solid #e0e0e0'}}>
                <Box display="flex" mb={1} sx={{alignItems: "center", justifyContent: "space-between"}}>
                    ...
                    <Tooltip
                        title="Reports are approved step by step, top to bottom. Bind a step to a role or a specific user; leave both empty to allow anyone with the Approve Report permission.">
                ...
                <Alert severity="info" sx={{mb: 3}}>
                    The last step is the final sign-off: its approver uploads the published PDF and is recorded as
                    the report's approver.
                </Alert>
                ...
                {data?.steps?.map((step, index) => (
                    <Paper
                        ...
                        sx={{
                            ...
                            borderLeftColor: index === data.steps.length - 1 ? 'primary.main' : 'grey.300',
                            borderRadius: 1
                        }}
                    >
                        <Grid container spacing={2} sx={{alignItems: "center"}}>
                            <Grid size={12} container>
                                <Grid size={{xs: 12, sm: 8}}>
                                    <Typography variant="subtitle2" gutterBottom>
                                        ...
                                        {index === data.steps.length - 1 && (
                                            <Chip label="Final sign-off" size="small" color="primary"
                                                  sx={{ml: 1}}/>
                                        )}
                                    </Typography>
                                </Grid>
                                <Grid size={{xs: 12, sm: 4}} sx={{textAlign: "right"}}>
                                    ...
                                    <Tooltip title="Move down" placement="top">
                                        <span>
                                            <IconButton size="small" disabled={index === data.steps.length - 1}
                                                        onClick={() => moveStep(index, 1)}>
                                    ...
                                    <Tooltip
                                        title={data.steps.length <= 1 ? "At least one step is required" : "Remove step"}
                                        placement="top"
                                    >
                                        <span>
                                            <IconButton
                                ...
                                <FormControlLabel
                                    control={
                                        <Checkbox
                                            ...
                                            onChange={(e) => handleStepChange(index, 'allow_self_approval', e.target.checked)}
                                        />
                                    }
                            ...
                            <Grid size={{xs: 12, sm: 6}}>
                                <SelectSearch
                                    onChange={(e) => handleStepChange(index, 'user', e.target.value)}
