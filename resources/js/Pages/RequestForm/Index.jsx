...
const Index = () => {
    ...
    const findRequestForm = useCallback(
        (id) => requestForms.data.find((requestForm) => requestForm.id === id),
        [requestForms],
    );

    const editRequestForm = useCallback(
        (id) => () => {
    ...
    const printRequestForm = useCallback(
        (id) => () => window.open(route('requestForms.show', id), '_blank'),
        [],
    );

    const deleteRequestForm = useCallback(
        (id) => () => {
    ...
    const handleDestroy = useCallback(
        () =>
            router.post(
                route('requestForms.destroy', requestForm.id),
                { _method: 'delete' },
                { onSuccess: handleCloseDeleteForm },
            ),
        [requestForm?.id, handleCloseDeleteForm],
    );
                    ...
                    if (row?.document?.id || row?.document?.hash)
                        cols.push(
                            <GridActionsCellItem
                                ...
                                href={route(
                                    'documents.show',
                                    row?.document?.id || row?.document?.hash,
                                )}
                            />,
                        );
    ...
    return (
        <>
            ...
            {openDeleteForm && (
                <DeleteForm
                    title={`${requestForm?.name} RequestForm`}
