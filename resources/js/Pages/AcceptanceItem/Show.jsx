...
const Show = ({
    ...
    canToggleReportless = false,
    canToggleSampleless = false,
}) => {
    const handleCheckWorkflow = () =>
        router.visit(route('acceptanceItems.check-workflow', acceptanceItem.id));

    const handleToggleReportless = () => {
        router.put(
            route('acceptanceItems.toggleReportless', {
    ...
    const handleToggleSampleless = () => {
        router.put(
            route('acceptanceItems.toggleSampleless', {
                acceptance: acceptanceItem.acceptance_id,
                acceptanceItem: acceptanceItem.id,
            }),
            {},
    ...
    return (
        <Container sx={{ p: '1em' }}>
            ...
            {acceptanceItem.patients.map((patient) => (
                <PatientInfo
                    patient={patient}
                    key={patient.id}
                    showDocuments
            ...
            {acceptanceItem.report ? (
                <ReportInfo report={acceptanceItem.report} />
            ) : (
                <Box sx={{ mt: 3 }}>
