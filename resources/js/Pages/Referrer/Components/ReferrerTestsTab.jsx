...
import { Alert, Snackbar, Tooltip, CircularProgress, Box, Typography } from '@mui/material';
...
const ReferrerTestsTab = ({ referrer }) => {
    ...
    const columns = useMemo(
        () => [
            ...
            },
            {
                ...
                sortable: false,
                renderCell: ({ row }) => (
                    <Typography variant="body2">{row?.price_type || 'N/A'}</Typography>
                ),
            },
            {
                field: 'id',
    ...
    const handleCopyFormClose = useCallback(
        (shouldReload = true) => {
        ...
        <>
            ...
            {!loading && referrerTests.data.length === 0 && (
                <Box sx={{ textAlign: 'center', py: 4 }}>
                    ...
                    <Button variant="contained" startIcon={<Add />} onClick={handleOpenAddNewForm}>
                        Add First Test
                    </Button>
                </Box>
            )}
            ...
            {/* Forms */}
            <AddPrice defaultValue={referrerTest} onClose={handleAddFormClose} open={openAddForm} />

            <CopyForm referrer={referrer} onClose={handleCopyFormClose} open={openCopyForm} />
