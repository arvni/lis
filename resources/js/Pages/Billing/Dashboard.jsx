...
const Dashboard = () => {
    ...
    const fetchCharts = useCallback((f = filters) => {
        ...
        axios
            .get(route('api.billing.dashboard.data'), {
                params: cleanParams(f),
                signal: abortRef.current.signal,
            })
            .then((r) => {
                setChartsData(r.data);
    ...
    const fetchTrend = useCallback((f = trendFilters) => {
        ...
        axios
            .get(route('api.billing.dashboard.trend'), {
                params: cleanParams(f),
            ...
            <Box sx={{ p: { xs: 1, sm: 2, md: 3 } }}>
                ...
                <Grid container spacing={2} sx={{ mb: 3 }}>
                    ...
                    <Grid size={{ xs: 12, sm: 6, lg: 3 }}>
                        <SummaryCard
                            title="Collected"
                            value={`OMR ${fmt(summary?.collected)}`}
                            icon={Payments}
                            color="success"
                            subtitle="Sum of payments"
                        />
                    </Grid>
                    <Grid size={{ xs: 12, sm: 6, lg: 3 }}>
                        <SummaryCard
                            title="Outstanding"
                            value={`OMR ${fmt(summary?.outstanding)}`}
                            icon={AccountBalance}
                    ...
                    <Grid size={{ xs: 12, sm: 6, lg: 3 }}>
                        <SummaryCard
                            ...
                            icon={Receipt}
                            color="info"
                            subtitle={`${summary?.acceptance_count ?? '—'} acceptances`}
                        />
                    </Grid>
                </Grid>
