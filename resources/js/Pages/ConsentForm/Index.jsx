...
const Index = () => {
    ...
    const findConsentForm = useCallback(
        (id) => consentForms.data.find((consentForm) => consentForm.id === id),
        [consentForms],
    );

    const editConsentForm = useCallback(
        (id) => () => {
    ...
    const deleteConsentForm = useCallback(
        (id) => () => {
    ...
    const handleDestroy = useCallback(
        () =>
            router.post(
                route('consentForms.destroy', consentForm.id),
                { _method: 'delete' },
                    ...
                    if (params?.row?.document?.id || params?.row?.document?.hash)
                        cols.push(
                            <GridActionsCellItem
                                ...
                                href={route(
                                    'documents.show',
                                    params?.row?.document?.id || params?.row?.document?.hash,
                                )}
                            />,
                        );
    ...
    return (
        <>
            ...
            {openDeleteForm && (
                <DeleteForm
                    title={`${consentForm?.name} Consent Form`}
