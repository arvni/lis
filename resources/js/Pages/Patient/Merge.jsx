...
const formatDob = (value) => {
    ...
    const date = new Intl.DateTimeFormat("en-GB", {year: "numeric", month: "short", day: "numeric"}).format(d);
    ...
    >
        ...
        <CardContent>
            <Stack direction="row" spacing={2} alignItems="center">
                ...
                />
                <Box sx={{minWidth: 0}}>
                    <Typography variant="h6" noWrap>{patient.fullName}</Typography>
            ...
            {!keep && (
                <Button size="small" sx={{mt: 1.5}} startIcon={<SwapHorizIcon/>} onClick={onKeep}>
...
const Merge = () => {
    ...
    useEffect(() => {
        ...
        setLoading(true);
        axios
            .get(route("api.patients.mergeCompare"), {params: {first_id: first.id, second_id: second.id}})
    ...
    const diffCount = useMemo(() => {
        ...
        const f = fields.filter((k) => (comparison.first.fields[k] ?? "") !== (comparison.second.fields[k] ?? "")).length;
        const m = metaFields.filter((k) => (comparison.first.meta[k] ?? "") !== (comparison.second.meta[k] ?? "")).length;
        return f + m;
    }, [comparison]);
    ...
    const submit = () => {
        ...
        transform(() => ({keep_id: keepPatient.id, remove_id: removePatient.id, attributes, meta}));
    ...
    return (
        <>
            ...
            <Card elevation={3} sx={{mb: 3, borderRadius: 2}}>
                <CardContent>
                    <Typography variant="overline" color="text.secondary">Step 1 — Select two patients</Typography>
                    ...
                    {sameSelected && (
                        <Alert severity="warning" sx={{mt: 2}}>Please select two different patients.</Alert>
                    )}
                </CardContent>
            </Card>
            ...
            {ready && (
                <>
                    ...
                    <Grid container spacing={2} alignItems="stretch" sx={{mt: 0.5, mb: 3}}>
                        ...
                        <Grid size={{xs: 12, md: 2}}
                            ...
                            <Tooltip title="Swap which patient is kept">
                                <Button
                                    variant="outlined"
                                    onClick={() => setKeepSide((s) => (s === "first" ? "second" : "first"))}
                        ...
                        <CardContent>
                            <Stack direction={{xs: "column", md: "row"}} spacing={1.5}
                                ...
                                <Box>
                                    ...
                                    <Typography variant="body2" color="text.secondary">
                                        {diffCount} field{diffCount === 1 ? "" : "s"} differ between the two records.
                                    </Typography>
                                </Box>
                                <Box sx={{display: "flex", flexWrap: "wrap", gap: 1, alignItems: "center"}}>
                                    <Button size="small" variant="outlined"
                                            onClick={() => preferAll("first")}>Use all from {comparison.first.fullName}</Button>
                                    <Button size="small" variant="outlined"
                                            onClick={() => preferAll("second")}>Use all from {comparison.second.fullName}</Button>
                                    <Button size="small" onClick={resetDefaults}>Smart defaults</Button>
                                    <FormControlLabel
                                        control={<Switch size="small" checked={onlyDiff}
                            ...
                            <Paper variant="outlined" sx={{p: 2, mb: 3, borderRadius: 2}}>
                                <Typography variant="subtitle2" sx={{mb: 1.5}}>Photo / ID image</Typography>
                                <Grid container spacing={2} alignItems="center">
                                                ...
                                                >
                                                    <Box
                                                        ...
                                                        sx={{
                                                            ...
                                                            borderRadius: 1, mb: 1, backgroundColor: "grey.100",
                                                        }}
                                                    />
                                                    <Stack direction="row" spacing={0.5} justifyContent="center"
                                                           alignItems="center">
                                                        <Radio checked={selected} size="small" sx={{p: 0}}/>
                                                        <Typography variant="caption" noWrap>{p.fullName}</Typography>
                                                    </Stack>
                                                </Paper>
                                            </Grid>
                                        );
                                    ...
                                    <Grid size={{xs: 12, sm: 6}}>
                                        <Stack direction="row" spacing={1.5} alignItems="center">
                                            ...
                                            <Avatar
                                                src={avatarUrl(
                                                    comparison[choices.avatar ?? keepSide].fields.avatar,
                                                    comparison[choices.avatar ?? keepSide].fields.gender,
                                                )}
                            ...
                            {errors && Object.keys(errors).length > 0 && (
                                <Alert severity="error" sx={{mt: 2}}>{Object.values(errors).join(" ")}</Alert>
                            )}
                        </CardContent>
                    </Card>
                    ...
                    <Paper
                        elevation={8}
                        sx={{
                            position: "sticky", bottom: 16, mt: 2, p: 2, borderRadius: 2,
                            display: "flex", flexWrap: "wrap", gap: 2,
                            alignItems: "center", justifyContent: "space-between",
                        }}
                    >
                        <Typography variant="body2">
                            Keeping <Chip size="small" color="success" label={keepPatient.fullName}/> · deleting{" "}
                            <Chip size="small" color="error" label={removePatient.fullName}/>
                        </Typography>
                        <Stack direction="row" spacing={1}>
                            <Button onClick={() => router.visit(route("patients.index"))}>Cancel</Button>
                            <Button
                                variant="contained"
                                color="primary"
                                startIcon={<MergeIcon/>}
            ...
            <Dialog open={confirmOpen} onClose={() => setConfirmOpen(false)} maxWidth="sm" fullWidth>
                ...
                <DialogContent>
                    <DialogContentText component="div">
                        This will permanently delete <strong>{removePatient?.fullName}</strong> and move all of its
                        acceptances, samples, invoices, payments, documents and other records to{" "}
                ...
                <DialogActions>
                    ...
                    <Button color="error" variant="contained" disabled={processing} onClick={submit}>
                        Merge &amp; Delete
                    </Button>
                </DialogActions>
            </Dialog>
...
const ComparisonTable = ({title, keys, labels, bucket, display, comparison, choices, keepSide, onlyDiff, onChoose, alwaysShow = []}) => {
                            ...
                            {rows.map((key) => {
                                ...
                                return (
                                    <TableRow key={key} hover>
                                        <TableCell sx={{fontWeight: 500}}>{labels[key] ?? key}</TableCell>
                                        <ChoiceCell selected={source === "first"}
                                                    onSelect={() => onChoose(key, "first")}>
                                            {display(key, a)}
                                        </ChoiceCell>
                                        <ChoiceCell selected={source === "second"}
...
Merge.layout = (page) => <AuthenticatedLayout auth={page.props.auth} children={page} breadcrumbs={breadCrumbs}/>;
