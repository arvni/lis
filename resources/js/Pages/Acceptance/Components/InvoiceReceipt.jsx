...
const OptimizedInvoiceReceipt = ({ acceptance, onPrint, showLogo }) => {
    ...
    const reportDays = Math.max(
        acceptance?.acceptanceItems?.tests?.reduce((max, item) => {
        ...
        (acceptance?.acceptanceItems?.panels || [])?.reduce((max, item) => {
            const turnaroundTime =
                item?.acceptanceItems?.reduce((pMax, acceptanceItem) => {
                ...
                <InvoiceContainer elevation={3} ref={printRef}>
                        ...
                        <Grid size={{ xs: 4 }}>
                            ...
                            <InfoItem
                                ...
                                value={Object.keys(acceptance?.howReport || {}).filter(
                                    (item) =>
                                        ['print', 'sms', 'whatsapp', 'sendToReferrer'].includes(
                                            item,
                    ...
                    <TableContainer component={Box} sx={{ mb: 1 }}>
                        <StyledTable size="small" padding="none">
                            ...
                            <TableBody>
                                {acceptance?.acceptanceItems?.panels?.length > 0 &&
                                    acceptance.acceptanceItems.panels.map((panel, index) => (
                                ...
                                {acceptance?.acceptanceItems?.tests?.length > 0 &&
                                    acceptance.acceptanceItems.tests.map((item, index) => (
                                        <TableRow key={`test-${item.id}-${index}`}>
                                            <TableCell>
                                                {index +
                                                    (acceptance?.acceptanceItems?.panels?.length ||
                                                        0) +
                                                    1}
                                            </TableCell>
                                            <TableCell
