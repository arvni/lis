...
const Add = ({ patient, canAddPrescription, maxDiscount }) => {
    ...
    const { data, setData, errors, setError, clearErrors, reset, processing } = useForm({
        patient,
        samplerGender: 1,
        out_patient: false,
    ...
    const validateForm = useCallback(
        (step) => {
            clearErrors();
            if (step === 2) return validateReferrer();
            if (step === 3) return validateTests();
            if (step === 4) return validateReporting();
            if (step === 5) return validateTests() && validateReferrer() && validateReporting();
        },
    ...
    const handleSubmit = useCallback(
        (formData, step) => {
            ...
            if (step > 0) {
                router.post(
                    route('acceptances.store', patient),
                    { ...formData, step },
                    {
                        onSuccess: () => {
                            reset();
