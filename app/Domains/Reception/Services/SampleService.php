...
class SampleService
{
    ...
    public function storeSample(SampleDTO $sampleDTO): Sample
    {
        $sample = $this->sampleRepository->findActiveSamples(Arr::pluck($sampleDTO->acceptanceItems, "id"), $sampleDTO->sampleTypeId);
        if (!$sample) {
            $deactivatedSample=$this->sampleRepository->findDeactivatedSamples(Arr::pluck($sampleDTO->acceptanceItems, "id"), $sampleDTO->sampleTypeId);
