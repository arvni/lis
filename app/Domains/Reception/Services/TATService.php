...
class TATService
{
    ...
    /**
     * Base acceptance constraint shared by getItems and getTestAnalytics.
     */
    ...
    /**
     * Active (non-reported, non-pooling, non-SERVICE) acceptance items.
     */
    ...
    public function getSummary(array $filters = []): array
    {
        ...
        // On-time completion rate: published items in last 30 days
    ...
    /**
     * Per-test analytics: avg/min/max calendar hours from order to publish.
     * Filters: a_from, a_to, a_test_id
     */
    public function getTestAnalytics(array $filters = []): array
    {
        ...
        $query = DB::table('acceptance_items')
            ...
            ->whereBetween('acceptance_items.created_at', [$from->startOfDay(), $to->copy()->endOfDay()])
            ->select(
                'tests.id as test_id',
    ...
    private function calcOnTimePct(Carbon $from, Carbon $to): ?int
    {
        ...
        $onTime = $items->filter(function (AcceptanceItem $item) {
            ...
            $publishedAt = $item->report?->published_at
                ? Carbon::parse($item->report->published_at)
                : null;
